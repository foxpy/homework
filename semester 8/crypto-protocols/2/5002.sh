...
set -euo pipefail
IFS=$'\n\t'
server="http://94.103.95.24:5002"
dictionary="$HOME/University/crypto proto/crackstation.txt"

curl "$server/db" 2>/dev/null > db
printf ".mode list\nSELECT password FROM user;\n" | sqlite3 db > hashlist
[[ "$(read -e -p 'Run hashcat session (slow, ~3min)? [y/N]> '; echo $REPLY)" == [Yy]* ]] && hashcat -m 0 hashlist "$dictionary" || true
cracked="$(hashcat -m 0 hashlist "$dictionary" --show || true)"
...
target_hash="$(echo "$sample" | tr ":" " " | awk '{print $1}')"
target_password="$(echo "$sample" | tr ":" " " | awk '{print $2}')"
