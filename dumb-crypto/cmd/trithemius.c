...
int main(int argc, char* argv[]) {
    ...
    if (qc_args_positionals_count(args) != 1) {
        ...
        if (strcmp(mode, "encrypt") == 0) {
            cipher_output = trithemius_cipher_encrypt(argv[qc_args_positionals_index(args)], 8, 4, key);
        } else {
            cipher_output = trithemius_cipher_decrypt(argv[qc_args_positionals_index(args)], 8, 4, key);
        }
        printf("%s\n", cipher_output);
        free(cipher_output);
    }
