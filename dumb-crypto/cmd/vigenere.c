...
int main(int argc, char* argv[]) {
    ...
    if (qc_args_positionals_count(args) != 1) {
        ...
        if (strcmp(mode, "encrypt") == 0) {
            cipher_output = vigenere_cipher_encrypt(argv[qc_args_positionals_index(args)], key);
        } else {
            cipher_output = vigenere_cipher_decrypt(argv[qc_args_positionals_index(args)], key);
        }
        printf("%s\n", cipher_output);
        free(cipher_output);
    }
