...
void run(Fl_Widget*, void* dat) {
    ...
    size_t n = std::atoi(run_data->n->value());
    char const* key = run_data->key->value();
    if (run_data->check_button->value() == 0) {
        char* encrypted = caesar_keyword_cipher_encrypt(input, n, key);
        ...
        char* decrypted = caesar_keyword_cipher_decrypt(input, n, key);
...
int main(int argc, char *argv[]) {
    ...
    run_data run_data{
            ...
            new Fl_Int_Input(240, 120, 60, 20, "n:"),
            new Fl_Input(380, 120, 60, 20, "Ключ:")
    };
